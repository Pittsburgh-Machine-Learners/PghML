...
def create_generator(generator_inputs, generator_outputs_channels):
    ...
    # encoder_1: [batch, 256, 256, in_channels] => [batch, 128, 128, args.ngf]
    with tf.variable_scope('encoder_1'):
        output = conv(generator_inputs, args.ngf, stride=2)
        layers.append(output)

    layer_specs = [
        args.ngf * 2,  # encoder_2: [batch, 128, 128, args.ngf] => [batch, 64, 64, args.ngf * 2]
        args.ngf * 4,  # encoder_3: [batch, 64, 64, args.ngf * 2] => [batch, 32, 32, args.ngf * 4]
        args.ngf * 8,  # encoder_4: [batch, 32, 32, args.ngf * 4] => [batch, 16, 16, args.ngf * 8]
        args.ngf * 8,  # encoder_5: [batch, 16, 16, args.ngf * 8] => [batch, 8, 8, args.ngf * 8]
        args.ngf * 8,  # encoder_6: [batch, 8, 8, args.ngf * 8] => [batch, 4, 4, args.ngf * 8]
        args.ngf * 8,  # encoder_7: [batch, 4, 4, args.ngf * 8] => [batch, 2, 2, args.ngf * 8]
        args.ngf * 8,  # encoder_8: [batch, 2, 2, args.ngf * 8] => [batch, 1, 1, args.ngf * 8]
    ]
    ...
    layer_specs = [
        (args.ngf * 8, 0.5),  # decoder_8: [batch, 1, 1, args.ngf * 8] => [batch, 2, 2, args.ngf * 8 * 2]
        (args.ngf * 8, 0.5),  # decoder_7: [batch, 2, 2, args.ngf * 8 * 2] => [batch, 4, 4, args.ngf * 8 * 2]
        (args.ngf * 8, 0.5),  # decoder_6: [batch, 4, 4, args.ngf * 8 * 2] => [batch, 8, 8, args.ngf * 8 * 2]
        (args.ngf * 8, 0.0),  # decoder_5: [batch, 8, 8, args.ngf * 8 * 2] => [batch, 16, 16, args.ngf * 8 * 2]
        (args.ngf * 4, 0.0),  # decoder_4: [batch, 16, 16, args.ngf * 8 * 2] => [batch, 32, 32, args.ngf * 4 * 2]
        (args.ngf * 2, 0.0),  # decoder_3: [batch, 32, 32, args.ngf * 4 * 2] => [batch, 64, 64, args.ngf * 2 * 2]
        (args.ngf, 0.0),  # decoder_2: [batch, 64, 64, args.ngf * 2 * 2] => [batch, 128, 128, args.ngf * 2]
    ]
    ...
    # decoder_1: [batch, 128, 128, args.ngf * 2] => [batch, 256, 256, generator_outputs_channels]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ngf', type=int, help='# generator filters', default=64)
    # parser.add_argument('--ndf', type=int, help='# discriminator filters', default=64)
